	...
	<div class="row">
		<div class="col-md-12">
			<section class="board-content">
					<%
					   ...
					   If BC_METHOD = "edit" Then
						  ...
						  If Rs.BOF = false AND Rs.EOF = false Then
							 ...
							 If B_FILE_NAME <> "" AND BC_TYPE <> "01" Then FILE_Script = "insert_img(0,'img','board','board_thumbnail','" & B_FILE_NAME & "','" & B_FILE_SIZE & "',600,305,233);"
							 If B_ADD1 <> "" AND BC_SEQ = "1" Then FILE_Script = "insert_img(1,'img','board','board_thumbnail','" & B_ADD1 & "','0',600,260,258);"
					...
					<input type="hidden" name="b_add1" id="b_add1" value="<%=B_ADD1%>">
					...
					<textarea id="bodytxt" name="bodytxt" style="display:none"></textarea>
					  ...
					  <fieldset class="board-write">
						<% If MEM_LEVEL = "99" OR (B_MEM_SEQ = 0 AND BC_METHOD = "edit") Then %>
							<div class="row">
							  <label class="col col-md-2"><i class="fa fa-check color-red"></i> 작성자</label>
							  <section class="col col-md-4">
								<label class="input">
								  <input type="text" name="b_guest_name" class="form-control" id="b_guest_name" value="<%=B_GUEST_NAME%>">
								</label>
							  </section>
							  <label class="col col-md-2"><i class="fa fa-check color-red"></i> 비밀번호</label>
							  <section class="col col-md-4">
								<label class="input">
								  <input type="password" name="b_guest_pwd" class="form-control" id="b_guest_pwd" value="<%=B_GUEST_PWD%>">
								</label>
							  </section>
							</div>
						<% Else %>
						...
						<% If BC_CATE <> "" Then %>
							<div class="row">
							  <label class="col col-md-2">카테고리</label>
							  <section class="col col-md-4">
								<label class="select">
						...
						<% If BC_HEADER <> "" Then %>
							<div class="row">
							  <label class="col col-md-2">구분/머리말</label>
							  <section class="col col-md-4">
								<label class="select">
						...
						<% End If %>
							<%If BC_SEQ = "2" Then%>
								<div class="row">
								  <label class="col col-md-2"><i class="fa fa-check color-red"></i> 세부카테고리</label>
								  <section class="col col-md-6">
									  <input type="text" name="b_add3" class="form-control" id="b_add3" value="<%=B_ADD3%>">
								  </section>
								</div>
							<%End If%>
							<div class="row">
						...
						<% If BC_SECRET = "1" AND B_PTSEQ = B_SEQ AND BC_METHOD <> "reply" Then %>
								  <label class="checkbox"><input type="checkbox" name="b_secret" id="b_secret" value="1" onclick="b_secret_chk()"><i></i> 비밀글</label>
						<% End If %>
								</div>
							  </section>
							</div>
							<%If BC_SEQ = "2" Then%>
								<div class="row">
								  ...
								  <section class="col col-md-2">
									  <input type="text" name="b_add2" class="form-control calandar" id="b_add2" value="<%=B_ADD2%>">
								  </section>
								</div>
								<div class="row">
								  <label class="col col-md-2"><i class="fa fa-check color-red"></i> 종료일자</label>
								  <section class="col col-md-2">
									  <input type="text" name="b_add4" class="form-control calandar" id="b_add4" value="<%=B_ADD4%>">
								  </section>
								</div>
							<%End If%>
						<% If BC_TYPE = "01" Then %>
							<div class="row">
							  <label class="col col-md-2"><i class="fa fa-check color-red"></i> 첨부파일</label>
							  <section class="col col-md-10">
								  <input type="text" name="file_name_view" class="form-control" id="file_name_view" value="<%=B_FILE_NAME%>" readonly style="display:inline-block; width:auto; vertical-align:middle;">
								  <button type="button" class="btn btn-default" onclick="upload_file(1,'file','board');">검색</button>
							  </section>
							</div>
							<%If BC_SEQ = "1" Then%>
								<div class="row">
								  <label class="col col-md-2"><i class="fa fa-check color-red"></i> 목록이미지<br>이미지 사이즈<br> [260 X 258]</label>
								  <section class="col col-md-4">
									<span id="show_img"><a href="javascript:upload_img(1,'img','board',800,260,258);"><img src="/exec/board/default/img/noimg.gif" border="0" /></a></span>
								  </section>
								</div>
							<%End If%>
						<% Else %>
							<div class="row">
							  <label class="col col-md-2"><i class="fa fa-check color-red"></i> 목록이미지<br>이미지 사이즈 [305 X 233]</label>
							  <section class="col col-md-4">
								<span id="show_img"><a href="javascript:upload_img(0,'img','board',800,305,233);"><img src="/exec/board/default/img/noimg.gif" border="0" /></a></span>
							  </section>
							</div>
						<% End If %>
...
<!--
  ...
  function b_secret_chk(){
    if (!form.b_secret.checked){
	    $("#secret_chk").hide();
    }else{
       $("#secret_chk").show();
	 }
