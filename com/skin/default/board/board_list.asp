	...
	<%
	   ...
	   Else
		  If Cstr(MEM_LEVEL) = "99" Then
			 Write_Script = "not_login('/member/login?','" & Rtn_Page & "');"
		  Else
	   ...
	   Dim LB_IMG(30),LB_IMG23(30),LB_WDATE(30),LB_MEM_SEQ(30),LB_MEM_NAME(30)
		  ...
		  Do until Rs.EOF
			 ...
			 Else
				...
				Else
				   ...
				   Else
					  If Cstr(MEM_LEVEL) = "99" Then
						 View_Script = "not_login('/member/login?','" & Rtn_Page & "');"
					  Else
			 ...
			 LB_LINK23(i) = "javascript:" & View_Script
			
			 If BC_SEQ = "6" Then
				 If TB_FILE_NAME <> "" Then 
					LB_IMG(i) = "<img class='img-responsive3' src='/file/board_thumbnail/" & TB_FILE_NAME & "' style='max-width:301px;'>" & vbNewLine
				 Else
					LB_IMG(i) = "<img class='img-responsive3' src='/skin/default/img/sub/noimage.jpg' style='max-width:301px;'>" & vbNewLine
				 End If
			 Else
				If TB_FILE_NAME <> "" Then LB_IMG(i) = "<img class='img-responsive3' src='/file/board_thumbnail/" & TB_FILE_NAME & "'>" & vbNewLine
			 End If
			 If TB_FILE_NAME <> "" Then LB_IMG23(i) = "/file/board_thumbnail/" & TB_FILE_NAME
	   ...
	   Rs.close
	%>
	<% If BC_TYPE = "01" OR BC_TYPE = "04" Then %>
<div class="board-wrap sub-box">
	...
	<%
			   If BC_NOTICE = "1" AND BC_TYPE = "01" Then
				  ...
				  If Rs.BOF = false AND Rs.EOF = false Then
					 Do until Rs.EOF
						...
						LT_TITLE = "<a href=""javascript:" & View_Script & """>" & TB_TITLE & "</a>"

						LT_MEM_SEQ = TB_MEM_SEQ
						LT_MEM_NAME = TB_MEM_NAME
						LT_READ_CNT = FormatNumber(TB_READ_CNT,0)
						LT_WDATE = f_chang_date(TB_WDATE)
						LT_FILE_NAME = TB_FILE_NAME
			   ...
			   If BC_CATE <> "" Then
				  With Response
					 .write "    <div class='third-navlist'>" & vbNewLine
					 .write "      <ul class='list'>" & vbNewLine
					 If InStr(BC_CATE,",") > 0 Then
						BSP_CATE = Split(BC_CATE,",")

						ALL_Cate_Script = "goBoard('" & SelfUrl & "','" & BC_SEQ & "','','list','" & page & "','" & B_SEQ & "','" & search_key & "','" & search_word & "');"
						
						If request("b_cate") = "" Then
						  .write "        <li class='active'>전체" & vbNewLine
						Else
						  .write "        <li><a href=""javascript:" & ALL_Cate_Script & """>전체</a>" & vbNewLine
						End If
			   ...
			   If BC_TYPE = "01" Then '일반형
				  With response
					 ...
					 .write "  <table class='tg tg-center mb40 hidden-sm hidden-xs'>" & vbNewLine
					 .write "    <colgroup>" & vbNewLine
					 .write "      <col width='10%'>" & vbNewLine
					 .write "      <col width='*'>" & vbNewLine
					 .write "      <col width='15%'>" & vbNewLine
					 .write "      <col width='15%'>" & vbNewLine
					 .write "      <col width='10%'>" & vbNewLine
					 .write "    </colgroup>" & vbNewLine
					 .write "    <thead>" & vbNewLine
					 .write "      <tr>" & vbNewLine
					 .write "        <th class='hidden-xs'>번호</th>" & vbNewLine
					 .write "        <th>제목</th>" & vbNewLine
					 .write "        <th>글쓴이</th>" & vbNewLine
					 .write "        <th>등록일</th>" & vbNewLine
					 .write "        <th class='last'>조회수</th>" & vbNewLine
					 .write "      </tr>" & vbNewLine
					 .write "    </thead>" & vbNewLine
					 ...
					 For i = 0 to pageSize - 1
						.write "      <tr>" & vbNewLine
						.write "        <td>" & LB_NUM(i) & "</td>" & vbNewLine
						.write "        <td>&nbsp;" & LB_TITLE(i) & "</td>" & vbNewLine
						.write "        <td>" & LB_MEM_NAME(i) & "</td>" & vbNewLine
						.write "        <td>" & LB_WDATE(i) & "</td>" & vbNewLine
						.write "        <td>" & LB_READ_CNT(i) & "</td>" & vbNewLine
						.write "      </tr>" & vbNewLine
					 Next
			   ...
			   %><!-- #include virtual = "/skin/default/page/goto_page.inc" --><%

			   With response
				  .write "<div class='text-center'>" & vbNewLine
				  .write "   <div class='page_nation'>" & vbNewLine
...
<!--
  ...
  function pass(method,b_seq){
    AjaxloadURL('/skin/default/board/password.asp?acturl=<%=ActCUrl%>&method=' + method + '&b_seq=' + b_seq, $('#pass-body'));
