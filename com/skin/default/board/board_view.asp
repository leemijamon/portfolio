...
<%
   ...
   If BC_LIST <> "1" Then
	  ...
	  If Rs.BOF	= false	AND	Rs.EOF = false Then
		 ...
		 BACK_TITLE	= "<div class=""th"">이전글</div><div class=""td""><span>"	& Rs("B_TITLE")	& "</span></div><div class=""date hidden-xs"">" &	f_chang_date(Rs("B_WDATE"))	& "</div>"
	  Else
		 BACK_LINK = "#"
		 BACK_TITLE	= "<div class=""th"">이전글</div><div class=""td""><span>이전 목록이 없습니다.</span></div>"
	  End If
	  Rs.close
	  ...
	  If Rs.BOF	= false	AND	Rs.EOF = false Then
		 ...
		 NEXT_TITLE	= "<div class=""th"">다음글</div><div class=""td""><span>"	& Rs("B_TITLE")	& "</span></div><div class=""date hidden-xs"">" &	f_chang_date(Rs("B_WDATE"))	& "</div>"
	  Else
		 NEXT_LINK = "#"
		 NEXT_TITLE	= "<div class=""th"">다음글</div><div class=""td""><span>이전 목록이 없습니다.</span></div>"
	  End If
	  Rs.close
   End If
	...
	<%End If%>
				<div class="custom_board_view mb20">
					<div class="custom_board_title">
						<p class="tit"><%=B_TITLE%></p>
						<ul class="info">
							<li><%=B_WDATE%></li>
							<%If B_FILE_NAME <> "" Then%><li><a href="/exec/file/download.asp?filepath=/file/board&filename=<%=B_FILE_NAME%>"><%=B_FILE_NAME%></a></li><%End If%>
							<li class="last"><%=MEM_NAME%></li>
						</ul>
					</div>
					<div class="custom_board_cont"><%=B_CONT%></div>
				</div>
					...
					<ul class="prev-next">
						...
						<li><a href="<%'=NEXT_LINK%>"><%'=NEXT_TITLE%></a></li>
					</ul>
					-->
					<div class="btn_list_wrap">
						<a class="btn_list" href="javascript:<%=List_Script%>">목록</a>
						<% If WRITE_BTN <> "0" Then %>
						<a class="btn_list" href="javascript:<%=Write_Script%>">글쓰기</a>
						<% End If %>
					</div>
					<% End If %>
