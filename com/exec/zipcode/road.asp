...
<title>우편번호검색 - 도로명주소의 우편번호</title>
...
//<![CDATA[
  ...
  function zip_search(){
        ...
        if (objF.key1.checked){
            var str = objF.keyword.value.split(" ").join("");
            ...
            if(objF.keyword.value.length<2){
            ...
            objF.method ="post";
            objF.action ="road.asp?key=road&f_key=<%=f_key%>";
        }
          ...
          if(objF.sido.value == ""){
            ...
            objF.method ="post";
            objF.action ="road.asp?key=dong&f_key=<%=f_key%>";
        }
          ...
          objF.method ="post";
          objF.action ="road.asp?key=bdnm&f_key=<%=f_key%>";
  ...
  function chgkey(num){
    ...
    if(document.getElementById("keyword").value.length==0){
      document.getElementById("keyword").style.background="url(/exec/zipcode/img/searchinputbg"+num+".gif) #fff no-repeat 2px 1px";
    }
  }
  ...
  function setDisplay(num){
    ...
    if(num==1){
      road="block";  dong="none"; bdnm="none";
    } else if(num==2){
      road="none";  dong="block"; bdnm="none";
    } else if(num==3){
      road="none";  dong="none"; bdnm="block";
    }
  ...
  function eventonblur(img_name){
    ...
    if(img_name.value == ""){
        img_name.style.background="url(/exec/zipcode/img/searchinputbg"+num+".gif) no-repeat 2px 1px";
  ...
  function init(){
    ...
    set_value(document.getElementById("f").key,'<%=request("key")%>');
  ...
  function add_openr(form){
    ...
    var eng_addr1 = form.eng_addr1.value;
    var eng_addr2 = form.eng_addr2.value;
...
<% If UBound(s_key) > 4 Then %>
      if(object.<%=s_key(4)%> != null) object.<%=s_key(4)%>.value = eng_addr1;
      if(object.<%=s_key(5)%> != null) object.<%=s_key(5)%>.value = eng_addr2;
<% End If %>
...
<div class="popup_addr_area">
  ...
  <div class="popup_addr_area_con">
    <ul class="popup_addr_tabmenu">
      <li><a href="road.asp?f_key=<%=f_key%>"><img src="/exec/zipcode/img/search_tab01_on.gif" alt="도로명주소의 우편번호"  id="Stab4"/></a></li>
      <li><a href="jibun.asp?f_key=<%=f_key%>" onmouseover="togSearchTab('1',event);" onmouseout="togSearchTab('1',event);" onfocus="togSearchTab('1',event);" onblur="togSearchTab('1',event);"><img src="/exec/zipcode/img/search_tab02_off.gif" alt="지번주소 우편번호" id="Stab1" /></a></li>
    </ul>
    <form name="f" id="f" action="road.asp" onsubmit="return zip_search();">
    <input type="hidden" name="f_key" value="<%=f_key%>" />
    <input type="hidden" name="roadNm" value="" />
    <input type="hidden" name="bldgMainNo" value="" />
    <input type="hidden" name="bldgSubNo" value="" />
    <input type="hidden" name="udrgrndYn" value="" />
    <input type="hidden" name="dongNm" value="" />
    <input type="hidden" name="lndnMainNo" value="" />
    <input type="hidden" name="lndnSubNo" value="" />
    <fieldset>
    <legend>상세검색</legend>
    <div class="ZiptopSchbox_sel">
      <input type="radio" name="key" id="key1" value="road" onclick="chgkey(1);" checked='checked' /><label for="key1"><strong>도로명+건물번호</strong></label>
      <input type="radio" name="key" id="key2" value="dong" onclick="chgkey(2);" /><label for="key2"><strong>동(읍/면/리)명+지번</strong></label>
      <input type="radio" name="key" id="key3" value="bdnm" onclick="chgkey(3);" /><label for="key3"><strong>건물명(아파트명 등)</strong></label>
    </div>
    <div class="ZiptopSchbox">
      <dl class="dlline">
        ...
        <dd><select id="sigungu" name="sigungu" class="plantext" style="width:120px;"  title="시군구 선택"><option value="" >전체</option></select></dd>
        <dd>
          <input name="keyword" id="keyword" type="text" size="17" class="popUpinput" value="<%=request("keyword")%>" title="검색어입력" style="IME-MODE:active;width:120px;<? if (!$keyword) { ?>background:url(/exec/zipcode/img/searchinputbg1.gif) #fff no-repeat 2px 1px;<?}?>" onclick="this.style.backgroundImage='';" onkeydown="this.style.backgroundImage='';" onblur="eventonblur(this);" />
          <input type="image" src="/exec/zipcode/img/popupbtn_sch.gif" alt="검색" />
        </dd>
      </dl>
      <ul class="search_infoma">
        <li id="road_show" style="display:block;">검색방법 : 서울시 중구 <strong>소공로 70</strong>&nbsp;&nbsp;예) '<strong>소공로</strong>(도로명) <strong>70</strong>(건물번호)'</li>
        <li id="dong_show" style="display:none;">검색방법 : 서울시 중구 <strong>충무로1가 21-1</strong>&nbsp;&nbsp;예) '<strong>서울</strong>' 선택 후 '<strong>충무로1가</strong>(동명) <strong>21-1</strong>(지번)'</li>
        <li id="bdnm_show" style="display:none;">검색방법 : 서울시 중구 <strong>중앙우체국</strong>&nbsp;&nbsp;예) '<strong>서울</strong>' 및 '<strong>중구</strong>' 선택 후 '<strong>중앙우체국</strong>(건물명)'</li>
      </ul>
    </div>
    </fieldset>
    </form>
    ...
    <div class="divTable1">
      ...
      <tbody>
<%
   SIDO_CD = "서울,부산,대구,인천,광주,대전,울산,세종,강원,경기,경남,경북,전남,전북,제주,충남,충북"
   SIDO_NAME = "서울특별시,부산광역시,대구광역시,인천광역시,광주광역시,대전광역시,울산광역시,세종특별자치시,강원도,경기도,경상남도,경상북도,전라남도,전라북도,제주특별자치도,충청남도,충청북도"
   SIDO_TABLE = "SEOUL,BUSAN,DAEGU,INCHEON,GWANGJU,DAEJEON,ULSAN,SEJONG,GANGWON,GYEONGGI,GYEONGNAM,GYEONGBUK,JEONNAM,JEONBUK,JEJU,CHUNGNAM,CHUNGBUK"
   ...
   With response
      If Request("keyword") <> "" Then
         ...
         Dim ZIPCODE_Table
         ZIPCODE_Table = "ZIPCODENEW..ZIPCODE_" & f_arr_value(SIDO_CD, SIDO_TABLE, SIDO)

         Dim ZIPCODE,ZIPSEQ,SIDO,SIDO_ENG,SIGUNGU,SIGUNGU_ENG,EUPMYEON,EUPMYEON_ENG,RI,ROADCODE
         Dim ROADNAME,ROADNAME_ENG,UNDERGROUNDYN,BUILDINGNUM,BUILDINGSUBNUM,DELIVERYNAME,DONGCODE,DONGNAME,LOTNUM,LOTSUBNUM
         Dim BUILDINGNAME,BUILDINGSEQ

         WHERE = "SIGUNGU = '" & SIGUNGU & "'"

         If KEY = "road" Then
            roadNm = request("roadNm")
            bldgMainNo = request("bldgMainNo")
            bldgSubNo = request("bldgSubNo")
            udrgrndYn = request("udrgrndYn")

            WHERE = WHERE & " AND ROADNAME = '" & roadNm & "'"
            If bldgMainNo <> "" Then WHERE = WHERE & " AND BUILDINGNUM = " & bldgMainNo
            If bldgMainNo <> "" AND bldgSubNo <> "0" Then WHERE = WHERE & " AND BUILDINGSUBNUM = " & bldgSubNo
            If udrgrndYn = "1" OR udrgrndYn = "2" Then WHERE = WHERE & " AND UNDERGROUNDYN = '" & udrgrndYn & "'"
         End If

         If KEY = "dong" Then
            dongNm = request("dongNm")
            lndnMainNo = request("lndnMainNo")
            lndnSubNo = request("lndnSubNo")

            WHERE = WHERE & " AND (EUPMYEON LIKE '%" & dongNm & "%' OR DONGNAME LIKE '%" & dongNm & "%' OR RI LIKE '%" & dongNm & "%')"
            WHERE = WHERE & " AND LOTNUM = " & lndnMainNo
            If lndnSubNo <> "0" Then WHERE = WHERE & " AND LOTSUBNUM = " & lndnSubNo
         End If

         If KEY = "bdnm" Then
            WHERE = WHERE & " AND BUILDINGNAME LIKE '%" & keyword & "%'"
         End If

         SQL = "SELECT * FROM " & ZIPCODE_Table & " WHERE " & WHERE & " ORDER BY ROADNAME, BUILDINGNUM, BUILDINGSUBNUM"
         'response.write SQL
         ...
         If Rs.BOF = false AND Rs.EOF = false Then
            Do until Rs.EOF
               ...
               ZIPSEQ = Rs("ZIPSEQ")
               SIDO = Rs("SIDO")
               SIDO_ENG = Rs("SIDO_ENG")
               SIGUNGU = Rs("SIGUNGU")
               SIGUNGU_ENG = Rs("SIGUNGU_ENG")
               EUPMYEON = Rs("EUPMYEON")
               EUPMYEON_ENG = Rs("EUPMYEON_ENG")
               RI = Rs("RI")
               ROADCODE = Rs("ROADCODE")
               ROADNAME = Rs("ROADNAME")
               ROADNAME_ENG = Rs("ROADNAME_ENG")
               UNDERGROUNDYN = Rs("UNDERGROUNDYN")
               BUILDINGNUM = Rs("BUILDINGNUM")
               BUILDINGSUBNUM = Rs("BUILDINGSUBNUM")
               DELIVERYNAME = Rs("DELIVERYNAME")
               DONGCODE = Rs("DONGCODE")
               DONGNAME = Rs("DONGNAME")
               LOTNUM = Rs("LOTNUM")
               LOTSUBNUM = Rs("LOTSUBNUM")
               BUILDINGNAME = Rs("BUILDINGNAME")
               BUILDINGSEQ = Rs("BUILDINGSEQ")
               ARA_CODE = Rs("ARA_CODE")

               ZIPCODE = Left(ZIPCODE,3) & "-" & Right(ZIPCODE,3)

               ROAD_ADDR = SIDO & " " & SIGUNGU
               If EUPMYEON <> "" Then ROAD_ADDR = ROAD_ADDR & " " & EUPMYEON
               If ROADNAME <> "" Then ROAD_ADDR = ROAD_ADDR & " " & ROADNAME
               If UNDERGROUNDYN = "1" Then ROAD_ADDR = ROAD_ADDR & " 지하"
               If UNDERGROUNDYN = "2" Then ROAD_ADDR = ROAD_ADDR & " 공중"

               ROAD_ADDR = ROAD_ADDR & " " & BUILDINGNUM
               If BUILDINGSUBNUM <> 0 Then ROAD_ADDR = ROAD_ADDR & "-" & BUILDINGSUBNUM

               ROAD_ADDR_ADD = ""
               If DONGNAME <> "" Then ROAD_ADDR_ADD = DONGNAME
               If BUILDINGNAME <> "" Then
                  If ROAD_ADDR_ADD <> "" Then ROAD_ADDR_ADD = ROAD_ADDR_ADD & ", "
                  ROAD_ADDR_ADD = ROAD_ADDR_ADD & BUILDINGNAME
               End If

               If ROAD_ADDR_ADD <> "" Then ROAD_ADDR = ROAD_ADDR & " (" & ROAD_ADDR_ADD & ")"

               JIBUN_ADDR = SIDO & " " & SIGUNGU
               If DONGNAME <> "" Then JIBUN_ADDR = JIBUN_ADDR & " " & DONGNAME
               If EUPMYEON <> "" Then JIBUN_ADDR = JIBUN_ADDR & " " & EUPMYEON
               If RI <> "" Then JIBUN_ADDR = JIBUN_ADDR & " " & RI
               JIBUN_ADDR = JIBUN_ADDR & " " & LOTNUM
               If LOTSUBNUM <> 0 Then JIBUN_ADDR = JIBUN_ADDR & "-" & LOTSUBNUM
               If BUILDINGNAME <> "" Then JIBUN_ADDR = JIBUN_ADDR & " " & BUILDINGNAME

               ENG_ADDR1 = SIDO_ENG
               ENG_ADDR2 = BUILDINGNUM
               If BUILDINGSUBNUM <> 0 Then ENG_ADDR2 = ENG_ADDR2 & "-" & BUILDINGSUBNUM

               If ROADNAME_ENG <> "" Then ENG_ADDR2 = ENG_ADDR2 & ", " & ROADNAME_ENG
               If EUPMYEON_ENG <> "" Then ENG_ADDR2 = ENG_ADDR2 & ", " & EUPMYEON_ENG
               If SIGUNGU_ENG <> "" Then ENG_ADDR2 = ENG_ADDR2 & ", " & SIGUNGU_ENG

               i = i + 1
               .write "        <form name='zip_code" & i & "' method='POST'>" & vbNewLine
               .write "        <input type='hidden' name='zipcode' value=""" & ZIPCODE & """>" & vbNewLine
               .write "        <input type='hidden' name='address' value=""" & ROAD_ADDR & """>" & vbNewLine
               .write "        <input type='hidden' name='eng_addr1' value=""" & ENG_ADDR1 & """>" & vbNewLine
               .write "        <input type='hidden' name='eng_addr2' value=""" & ENG_ADDR2 & """>" & vbNewLine
               .write "        <input type='hidden' name='ara_code' value=""" & ARA_CODE & """>" & vbNewLine

               .write "            <tr>" & vbNewLine
               .write "              <td style='color:#252525;'>" & ZIPCODE & "</td>" & vbNewLine
               .write "              <td style='text-align:left;padding-left:5px;'><font style='color:#0958db;'>" & ROAD_ADDR & "</font><br />" & JIBUN_ADDR & "</td>" & vbNewLine
