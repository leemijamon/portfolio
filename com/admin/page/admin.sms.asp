...
<%
   'SMS 잔여량 구함
   SMS_CNT = Sms_Count(SMS_ID,SMS_PWD)

   'LMS 잔여량 구함
   LMS_CNT = Lms_Count(LMS_ID,LMS_PWD)
...
<fieldset>
  <label class="textarea">
    <textarea rows="5" name="msg" data-parsley-maxlength="<%=CUT_CNT%>" onkeydown='CalByte(this)' onchange='CalByte(this)' onkeyup='CalByte(this)' required><%=SMS_CONSULT_ADM%></textarea>
  </label>
  <div style="text-align:right; font-size:11px; font-family: 돋움;"><strong><span id='msglen'>0</span></strong> / <%=CUT_CNT%> bytes</div>
...
<!--
  ...
  function CalByte(tg){
    ...
    for(i=0; i<strLen; i++) {
      ...
      if(byteIs > <%=CUT_CNT%>){ // 3페이지까지
        alert('[안 내] <%=CUT_CNT%>byte를 초과하실 수 없습니다.');
